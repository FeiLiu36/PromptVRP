...
from VRProblemDef import get_random_problems, augment_xy_data_by_8_fold
...
class VRPEnv:
    ...
    def load_problems(self, batch_size,task_params, aug_factor=1, load_problem= False):
        self.batch_size = batch_size

        #print(task_params)
        if load_problem == True:
            depot_xy, node_xy, node_demand = task_params
            if len(depot_xy[0])==2:
                depot_xy = torch.unsqueeze(depot_xy,1)
            ...
            depot_xy, node_xy, node_demand, capacity = get_random_problems(batch_size,  task_params[0], num_modes=task_params[1], cdist=task_params[2], distribution='gaussian_mixture', problem="cvrp")
