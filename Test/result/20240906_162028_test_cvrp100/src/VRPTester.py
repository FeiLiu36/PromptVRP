...
class VRPTester:
    ...
    def _test_one_batch(self, test_data):
        ...
        with torch.no_grad():
            self.env.load_problems(batch_size, test_data, aug_factor=aug_factor,load_problem=True)
        ...
        for i in range(self.model_params['top_k']):
            ...
            if i == 0:
                max_pomo_reward_all = max_pomo_reward
                no_aug_reward = torch.unsqueeze(max_pomo_reward[0, :], 0)
            else:
                max_pomo_reward_all = torch.cat((max_pomo_reward_all,max_pomo_reward),dim=0)
                no_aug_reward = torch.cat((no_aug_reward,torch.unsqueeze(max_pomo_reward[0, :], 0)),dim=0)


        max_no_aug_reward, _ = no_aug_reward.max(dim=0)
        no_aug_score = -max_no_aug_reward.float()  # negative sign to make positive value
        no_aug_score_mean = no_aug_score.mean()

        print(no_aug_reward.shape)
        print(max_pomo_reward_all.shape)

        max_aug_pomo_reward, _ = max_pomo_reward_all.max(dim=0)  # get best results from augmentation
        # shape: (batch,)
    ...
    def _solve_cvrplib(self, path):
        ...
        #loc_scaler = 1000
        # print(original_locations.shape)
        # print(original_locations[0,:,0])
        # input()
        loc_scaler = np.max(original_locations[0,:,0])-np.min(original_locations[0,:,0])
        locations = (original_locations - np.min(original_locations[0,:,0]))/ loc_scaler  # [1, n+1, 2]: Scale location coordinates to [0, 1]
        ...
        outfile = open(self.tester_params['dir_output'],'a')
        outfile.write("{}     {:.3f}     {:.3f} \n".format(self.tester_params['test_set_path'] , no_aug_score.item(),aug_score.item()))
        # print(self.tester_params['dir_output'], score_AM.avg,aug_score_AM.avg)
        # input()
