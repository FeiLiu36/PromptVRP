...
class VRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        # shape: (batch, problem+1, embedding)
        #print(self.encoded_nodes[0])
        #self.decoder.set_kv(self.encoded_nodes)

        #print(torch.mean(self.promptpool.prompt,dim=2))
        #print(torch.mean(self.promptpool.prompt_key,dim=1))

        #node_size = self.encoded_nodes.size(1)

        #encoded_mean = torch.mean(self.encoded_nodes, dim=1)
        # print(self.encoded_nodes[0,:,0])
        # print(encoded_mean[0][0])

        #print(self.encoded_nodes.size())
        #encoded_nodes_map =  self.linear(self.encoded_nodes)
        #encoded_nodes_scale = self.encoded_nodes
        #print(encoded_nodes_prompt[0])
        ...
        #out = self.promptpool(self.prompt_id_select)
        # shape: (batch, problem+1, embedding)

        # file = open('embedding.dat','a')
        # #print(torch.mean(torch.mean(encoded_nodes_prompt,dim=1),dim=0).shape)
        # embedding_out = torch.mean(torch.mean(encoded_nodes_prompt,dim=1),dim=0).cpu().numpy()
        # for i in range(len(embedding_out)):
        #     file.write(' %.5f ' % (embedding_out[i]))
        # file.write('\n')


        #out_sim = out['reduce_sim']

        prompt_embeddin_mean = out['prompt_embedding']


        #self.decoder.set_prompt_embedding(out['prompt_embedding'])
        # print(self.promptpool.prompt.shape)
        # print(out['prompt_idx'] )
        # print(torch.sum(self.promptpool.prompt,dim=2))
        #print(out['prompt_idx'][0])
        #print(prompt_embeddin_mean.shape)
        #print(prompt_embeddin_mean.sum(0).sum(1).sum(1))

        self.encoded_nodes, encoded_nodes_prompt = self.encoder(depot_xy, node_xy_demand, prompt_embeddin_mean)


        #self.decoder.set_prompt_embedding(out['prompt_embedding_2'])
        ...
        self.prompt_id_last = torch.unique(out['prompt_idx'])
        
        #print(self.prompt_id_last)
